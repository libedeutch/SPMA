...
Rcpp::List spmaC(double alpha, double epi_abs, double epi_rel, double rho, double lambda, int iter, NumericMatrix covF1, NumericMatrix covF2, NumericMatrix covF3, arma::mat augIMat, int p, int M, IntegerVector setN, arma::mat thetaF){
  ...
  arma::mat theta_s(3,p);
  theta_s = thetaF; 
  arma::mat thetaM(3,p*(iter+1));
  arma::mat alphaM(3,p*(iter+1));
  arma::mat wM(3,p*(iter+1));
...
//  Rcout<<thetaFvec << "&&&&&&&&&" ;
  ...
  for(int it=1; it<iter; it++){
    // every iteration 
    // first update theta
    arma::mat flatten_alphaM = alphaM.submat(0,p*(it-1),2,(p*it)-1).as_row();
    arma::mat flatten_wM = wM.submat(0,p*(it-1),2,(p*it)-1).as_row();
    ...
    // Rcout<< tmp.n_rows<< "****";
     thetaM.submat(0,p*it,2,(p*(it+1))-1) = tmp.t() ;
     // Rcout<<thetaM<<"*******";
     // update alpha 
     ...
     for(int jj=0; jj<p;jj++){
       ...
       arma::mat AlphajTmp = thetaMhat.submat(0,it*p+jj,2,it*p+jj) + wM.submat(0,p*(it-1)+jj,2,p*(it-1)+jj) ; 
      ...
      // double test = 0.00;
       ...
       if((1-secTerm)>=1){
         alphaM.submat(0,it*p+jj,2,it*p+jj)  = 0*secTerm*AlphajTmp;
       }
       else{
         alphaM.submat(0,it*p+jj,2,it*p+jj)  = secTerm*AlphajTmp;
       }
     ...
     // update of wM 
      ...
      arma::mat thetaMhat2 = alpha*thetaM.submat(0,p*it,2,(p*(it+1))-1)+ (1-alpha)*alphaM.submat(0,p*(it-1),2,(p*(it))-1);
     
     arma::mat wMtmp = wM.submat(0,p*(it-1),2,(p*it)-1) + thetaMhat2 - alphaM.submat(0,p*it,2,(p*(it+1))-1) ;
     
     wM.submat(0,p*it,2,(p*(it+1))-1) = wMtmp;
