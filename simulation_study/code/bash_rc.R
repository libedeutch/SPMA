...
option_list<- list(
  ...
  make_option("--p", default = 10, help = "Number of covariates"),
  ...
  make_option("--idxend",default=500,help = "idx of end"),
...
for(i in opt$idx:opt$idxend){
  print(i)
  print(seed[i]) # 222T needs seed[i,2]
  tt = generate_data(N = c(opt$n1,opt$n2,opt$n3), p= opt$p,q=0.50, seed = seed[i],rightCensor = opt$rc, correlate = opt$correlate, intervalCensor =FALSE)
  ...
  re2 = trainSPMA(tt,iter = 1000,rightCensor = TRUE, intervalCensor = FALSE,m = 5)
  ...
  #result[[1]] = rmseO
  result[[1]] = re2
  save(result, tt, file = paste0("~/metaADMM/rcJun5/sample_",opt$n1,"_",opt$n2,"_",opt$n3,"_rc_",opt$rc,"_cor_",opt$correlate,"_p_",opt$p,"_", i,".RData"))
}
